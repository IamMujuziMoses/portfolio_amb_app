...
class _NewRideScreenState extends State<NewRideScreen> with TickerProviderStateMixin{
  ...
  void getRideLiveLocationUpdates() {
    ...
    rideStreamSubscription = Geolocator.getPositionStream().listen((Position position) async {
      ...
      setState(() {
        CameraPosition cameraPosition = new CameraPosition(target: nPosition, zoom: 17);
                            ...
                            children: <Widget>[
                              ...
                              Padding(
                                ...
                                child: Row(
                                  ...
                                  children: <Widget>[
                                    Container(
                                      child: Text(widget.rideDetails.riderName, style: TextStyle(
                                        fontFamily: "Band Bold",
                                    ...
                                    Padding(
                                      ...
                                      child: GestureDetector(
                                        onTap: () => launch(('tel:${widget.rideDetails.riderPhone}')),
                                        child: Container(
                                      ...
                                      children: <Widget>[
                                        Row(
                                          children: <Widget>[
                                            ...
                                            Expanded(
                                              child: Container(
                                                child: Text(widget.rideDetails.pickUpAddress, style: TextStyle(
                                                  fontSize: 2.5 * SizeConfig.textMultiplier,
                                                ), overflow: TextOverflow.ellipsis,),
                                              ),
                                            )
                                ...
                                child: RaisedButton(
                                  ...
                                  onPressed: () async {
                                    if (status == "accepted") {
                                      status = "arrived";
                                      String rideRequestId = widget.rideDetails.rideRequestId;
                                      await databaseMethods.updateRideRequestDocField({"status": status}, rideRequestId);
                                      ...
                                      await getPlaceDirections(widget.rideDetails.pickUp, widget.rideDetails.dropOff);
  ...
  Future<void> getPlaceDirections(LatLng pickUpLatLng, LatLng dropOffLatLng) async {
    showDialog(
      context: context,
    ...
    Marker dropOffLocMarker = Marker(
      ...
      infoWindow: InfoWindow(title: status == "enRoute"
          ? "DropOff Location"
          : "PickUp Location", snippet: status == "enRoute"
  ...
  void acceptRideRequest() async{
    ...
    databaseMethods.getDriverByUid(currentDriver.uid).then((val) async{
      ...
      if (snap.docs[0].get("history") != null) {
        ...
        await databaseMethods.updateDriverDocField({"history": histMap}, currentDriver.uid);
      } else {
  ...
  endTrip() async {
    ...
    String rideRequestId = widget.rideDetails.rideRequestId;
    await databaseMethods.updateRideRequestDocField({"status": "ended"}, rideRequestId);
